...
| ↳ Mario-A | Sprite source lead / designated integrator | V07-SPR-MA repair | WORKING | Reopened narrow source lane after visual review reproduced a pathological LEFT-IMG-10 Topete extraction: slot 6 ~1% normal body area while slot 7 contains two poses. Repair source layout only, then re-hand off exact SHA to Mario-B. |
| ↳ Mario-B | El Toro sprite package | V07-SPR-MB | HANDOFF_READY_BILATERAL_ANCHOR_REVIEW | Exact candidate `d6b41ff...` GREEN on run `35742812047`; bilateral non-mirrored package + 26/26 keys + FX + previews + JSON/SVG reviews + self-contained `bilateral-anchor-review.html`. Only explicit visual anatomical-anchor verification remains before runtimeLoadable:true. |
