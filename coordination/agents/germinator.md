...
- For generated runtime assets, treat a package-level `runtimeLoadable` result as necessary but not sufficient: release QA must also prove the exact candidate exposes the owning feature through default product registries, materializes build-served assets, and can reach/load the package through the live runtime path. A green generator plus a green generic loader can still compose into a non-previewable product.
- When renderer migrations coexist with legacy capture tooling, validate that the evidence actually consumes the target backend/assets and inspect the resulting pixels. A green screenshot workflow can capture a legacy renderer or an async pre-load frame; keep a deterministic manifest/atlas visual compositor as a fallback for exact-asset QA.
