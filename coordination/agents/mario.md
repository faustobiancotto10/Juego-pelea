...
- For component-extracted sprite sheets, frame-count and non-empty checks are not enough: compare per-slot content mass against the animation's own distribution before package or anchor certification. A nearly empty slot beside an oversized neighbor is a strong signal that two authored poses collapsed into one component group even when nominal slot count still passes.
- When promoting a derived sprite package, treat `runtimeLoadable` as a package-level gate only. Live cutover also requires the approved product lineage to select the sprite backend, a default package-registry entry, build-served manifest/atlas bytes, and fight-scoped load/release/rematch verification. Recompose backend deltas onto the approved product lineage instead of merging an older product tree wholesale.
