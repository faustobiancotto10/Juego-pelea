...
export function generateSpriteSourceEvidence({ sourceDir, outDir, facing = 'right' }) {
  ...
  const unsupportedPngs = [];
  const emptyFrames = [];
  const canvasEdgeClipping = [];
  const cellBoundaryTouches = [];
  const sheets = [];
  const frames = [];
  ...
  for (const spec of sheetSpecs) {
    ...
    if (actualHash !== spec.hash) {
      hashMismatches.push({ file: spec.file, expected: spec.hash, actual: actualHash });
    }

    let image;
    try {
      image = parseRgbaPng(bytes);
    } catch (error) {
      unsupportedPngs.push({
        file: spec.file,
    ...
    for (const extracted of extraction.frames) {
      ...
      frames.push({
        ...
        bbox: extracted.bbox,
        extraction: {
          method: 'alpha-components',
          contentAlpha: 32,
          components: extracted.components,
          crossesNominal: extracted.crossesNominal,
          pixelIsolation: 'component-owned-rgba-v1',
        },
      });
    }
    ...
    sheets.push({
      ...
      expectedFrames: spec.expectedFrames,
      extractedFrames,
      hasVisiblePixels: extraction.visiblePixels > 0,
      hasTransparentPixels: extraction.transparentPixels > 0,
    });
  }
  ...
  const rejectedAlternatePresent =
    facing === 'right' && names.has(REJECTED_ALTERNATE);
  ...
  const manifest = {
    ...
    shippingStatus:
      facing === 'right'
        ? 'pilot-only-left-facing-blocked'
        : 'pilot-only-anchor-blocked',
    authority: [
    ...
    runtimeBoundary:
      'Runtime consumes normalized derived frames/atlases and this transform contract; source sheets remain authoring-only and must never be loaded as fighter textures.',
    leftFacingGate:
      facing === 'right'
  ...
  writeFileSync(
    join(outputRoot, 'NORMALIZATION_MANIFEST.json'),
    JSON.stringify(manifest, null, 2) + '\n',
  );
  writeFileSync(
    join(outputRoot, 'NORMALIZED_PREVIEW.svg'),
    renderNormalizedPreview(manifestFrames, sheets, normalization, facing),
  );

  const bodyFrameCount = frames.filter((frame) =>
    /^IMG-(0[1-9]|1[0-2])$/.test(frame.sheetId)
  ).length;
  ...
  return {
    ...
    bodyFrameCount,
    fxFrameCount,
    totalPreviewFrames: frames.length,
    hashMismatches,
    rejectedAlternatePresent,
    emptyFrames,
    canvasEdgeClipping,
...
export function generatePixelIsolatedFrameSet({ sourceDir, facing = 'right' }) {
  ...
  if (facing === 'right' && names.has(REJECTED_ALTERNATE)) {
  ...
  for (const spec of sheetSpecs) {
    ...
    if (actualHash !== spec.hash) {
      throw new Error(
        'source hash mismatch for ' + spec.file +
        ': expected ' + spec.hash + ' actual ' + actualHash
      );
    }
    sourceHashes[spec.id] = actualHash;

    const image = parseRgbaPng(bytes);
    const extraction = extractPixelIsolatedFrames(
      image,
      spec.rows,
      spec.cols,
      { contentAlpha: 32, clipAlpha: 128 },
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i += 1) {
    ...
    else if (argv[i] === '--facing') args.facing = argv[++i];
    else throw new Error('unknown argument ' + argv[i]);
  }

  if (!args.sourceDir || !args.outDir) {
    throw new Error(
      'usage: node scripts/el-toro-sprite-source-pipeline.mjs --source-dir <dir> --out-dir <dir> [--facing right|left]'
    );
  }

  args.facing = args.facing ?? 'right';
  if (args.facing !== 'right' && args.facing !== 'left') {
    throw new Error('unsupported sprite facing ' + args.facing);
  }
